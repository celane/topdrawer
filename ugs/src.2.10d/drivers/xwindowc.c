...
/* colormap mod  20070226  lane@duphy4.physics.drexel.edu */
...
void fxopen_ (int *xsiz, int *ysiz )
{
  ...
  *xsiz = disw ; *ysiz = dish ;
  ...
  status = XLookupColor(dpy,DefaultColormap(dpy,scr),"white",  &dum_color,&app.color[1]);
  status = XLookupColor(dpy,DefaultColormap(dpy,scr),"red",    &dum_color,&app.color[2]);
  status = XLookupColor(dpy,DefaultColormap(dpy,scr),"green",  &dum_color,&app.color[3]);
  status = XLookupColor(dpy,DefaultColormap(dpy,scr),"blue",   &dum_color,&app.color[4]);
  status = XLookupColor(dpy,DefaultColormap(dpy,scr),"yellow", &dum_color,&app.color[5]);
  status = XLookupColor(dpy,DefaultColormap(dpy,scr),"magenta",&dum_color,&app.color[6]);
  status = XLookupColor(dpy,DefaultColormap(dpy,scr),"cyan",   &dum_color,&app.color[7]);
  status = XLookupColor(dpy,DefaultColormap(dpy,scr),"black",  &dum_color,&app.color[8]);
...
void fxcolor_ (int *color )
{
  ...
  XSetForeground (dpy, gc, app.color[*color].pixel ) ;
}
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
void fxclear_()
{
...
main ()
{
    ...
    for (ic = 1; ic < 8; ic++) {
      printf ("color %d =0x%06x ",ic,app.color[ic].pixel);
      fxcolor_(&ic);
      i = 2;
      j = 1;
      fxsetstyle_ ( &i , &j ) ;
